...
def registration_email_text(token, js):
    return (
        ...
        "Want the paper-plane icon on your bookmark? Browsers don't give dragged\n"
        "bookmarklets an icon, so: bookmark https://send2me.app/ itself\n"
        "(Ctrl/Cmd+D), then edit that bookmark and replace its address with the\n"
        "entire code below - it keeps the site's icon.\n\n"
        "On your phone:\n"
        "1. Save any page as a bookmark and name it send2me.\n"
        "2. Edit the bookmark and replace its address with the entire code below.\n\n"
        "The code (for the icon trick and for phones):\n\n"
        "%s\n\n"
